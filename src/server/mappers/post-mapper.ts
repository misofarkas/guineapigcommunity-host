import {$Enums, comment, post, user, vote_post, vote_comment} from ".prisma/client";
import {z} from "zod";
...
import {toTagTypesPrisma, toTagTypesZod} from "@/server/mappers/tag-type-mapper";
...
export const toPostDetailZod = (entity: {user: user, comment: ({user: user, vote_comment: vote_comment[]} & comment)[], vote_post: ({user: user} & vote_post)[]} & post) => {
...
export const toPostPrisma = (post: z.infer<typeof postFormSchema>, createdById: number) => {
    return {
        title: post.title,
        text: post.text,
